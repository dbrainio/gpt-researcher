...
class CustomLogsHandler:
    ...
    def __init__(self, websocket, task: str):
        ...
        sanitized_filename = sanitize_filename(f"task_{int(time.time())}_{task[:10]}")
...
async def handle_start_command(websocket, data: str, manager):
    ...
    # Cleanup generated files after sending paths to the client
    cleanup_generated_files(file_paths)
...
def cleanup_generated_files(file_paths: Dict[str, str]) -> None:
    """
    Remove generated report files from disk once they've been sent to the client.

    The deep-research backend returns URL-encoded paths for PDF/DOCX/MD files and
    a regular relative path for the JSON log file. We normalize all of them before
    attempting deletion.
    """
    logger.info("cleanup_generated_files: deleting generated files: %s", file_paths)
    for _, path in file_paths.items():
        ...
        try:
            ...
            logger.warning(f"Failed to delete generated file %s: %s", path, e)
        ...
        while True:
            try:
                ...
                # Normalize command detection by checking startswith after stripping whitespace
                    ...
                    error_msg = (
                        f"Error: Unknown command or not enough parameters provided. Received: '{data[:100]}...'"
                        if len(data) > 100
                        else f"Error: Unknown command or not enough parameters provided. Received: '{data}'"
                    )
